...
<article {{ $attributes->class('group border-b border-white/10 py-7 last:border-b-0') }}>
    <div class="flex items-start justify-between gap-4 sm:gap-6">
        ...
        <div class="min-w-0 flex-1">
            ...
            <div class="mb-3 flex min-w-0 items-center gap-2 text-sm text-gray-400">
                <a wire:navigate href="{{ route('users.show', $post->author) }}"
                    class="size-7 shrink-0 overflow-hidden rounded-full transition hover:opacity-70 sm:size-8">
                ...
                <a wire:navigate href="{{ route('users.show', $post->author) }}"
                    class="min-w-0 truncate font-medium text-gray-200 transition hover:text-white hover:underline">
            ...
            <div class="mt-3 flex items-center">
                <x-ui.avatar-group :users="$clapper" class="mt-4" />

                {{-- Desc --}}
                <div class="ml-2 text-xs text-gray-400">
                    Clapped by
                    <a wire:navigate href="{{ route('users.show', $clapper->first()->username) }}" class="font-semibold hover:text-white">{{ $clapper->first()->username }}</a>
                    @if ($clapper->count() > 1)
                    and
                    <b class="font-semibold">{{ $post->claps->sum('count') - 1 }} others</b>
                    @endif
                </div>
            </div>
        </div>
        ...
        <a wire:navigate href="{{ route('posts.show', $post->slug) }}"
            class="block h-24 w-24 shrink-0 overflow-hidden rounded-xl bg-white/5 sm:h-36 sm:w-36 md:w-48">
            @if ($post->images->first())
            <img src="{{ str($post->images->first()->url)->contains('http') ? $post->images->first()->url : config('app.url').$post->images->first()->url }}"
                alt="Gambar Post" class="h-full w-full object-cover transition duration-300 group-hover:scale-105">
            @else
