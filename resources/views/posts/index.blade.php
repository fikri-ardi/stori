    ...
    <div class="flex flex-wrap justify-between space-y-10">
        @foreach ($posts as $post)
            <article class="md:max-w-[368px] flex flex-col justify-between">
                ...
                {{-- Content --}}
                <div class="mb-4">
                    <div class="flex items-center space-x-4 text-xs text-gray-400 mb-4 font-semibold">
                        {{-- Date --}}
                    ...
                    <a href="{{ route('posts.show', $post->slug) }}">
                        <h2 class="text-2xl font-semibold mb-4">{{ str($post->title)->lower()->ucfirst() }}</h2>
                        <p class="text-gray-400 leading-relaxed text-justify">{{ $post->excerpt }}</p>
                    </a>
                </div>
            
                {{-- Author --}}
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 text-sm rounded-full overflow-hidden bg-red-400 flex items-center justify-center font-semibold text-white">
